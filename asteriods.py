...
class Sprite:
    ...
    def update(self):
        ...
        if self.pos[0] >= Constants.WIDTH - 1:
            self.pos[0] = 0
        elif self.pos[0] <= 0:
            self.pos[0] = Constants.WIDTH - 1

        if self.pos[1] >= Constants.HEIGHT - 1:
            self.pos[1] = 0
        elif self.pos[1] <= 0:
            self.pos[1] = Constants.HEIGHT - 1
    ...
    def is_collison(self, pos, imageInfo):
        if self.bexplode != True:  # nur wenn ich nicht gerade explodiere
            distance = Utility.dist(self.pos, pos)
...
class Ship:
    ...
    def update(self):
        ...
        # update accelaration
        if self.thrust:
            forward = Utility.angle_to_vector(self.angle)
...
class Asteroids:
    def __init__(self):
        ...
        self.ship = Ship([Constants.WIDTH / 2, Constants.HEIGHT / 2], [0, 0], 0)
    ...
    def keydown(self, key):
        ...
        if key == simplegui.KEY_MAP["space"]:
            forward = Utility.angle_to_vector(self.ship.angle)
    ...
    def draw(self, canvas):
        # animiate background
        self.time += 1
        wtime = (self.time / 4) % Constants.WIDTH
        ...
        canvas.draw_image(Utility.debris_image, center, size, (wtime - Constants.WIDTH / 2, Constants.HEIGHT / 2),
                          (Constants.WIDTH, Constants.HEIGHT))
        canvas.draw_image(Utility.debris_image, center, size, (wtime + Constants.WIDTH / 2, Constants.HEIGHT / 2),
                          (Constants.WIDTH, Constants.HEIGHT))

        canvas.draw_text("Score: " + str(self.score), [Constants.WIDTH - Constants.WIDTH / 5, 0 + Constants.HEIGHT / 5],
                         20,
                         "White")
        canvas.draw_text("Lives: " + str(self.lives), [0 + Constants.WIDTH / 10, 0 + Constants.HEIGHT / 5], 20, "White")

        # draw splash screen if not started
        if not self.started:
            canvas.draw_image(Utility.splash_image, Utility.splash_info.get_center(),
                              Utility.splash_info.get_size(), [Constants.WIDTH / 2, Constants.HEIGHT / 2],
                              Utility.splash_info.get_size())
        else:
            if self.spawn_rock == True:
                if len(self.rocks) <= 2:
                    rock = Sprite([random.randrange(0, Constants.WIDTH + 1), random.randrange(0, Constants.HEIGHT + 1)],
    ...
    def run(self):
        self.frame = simplegui.create_frame("RiceRocks", Constants.WIDTH, Constants.HEIGHT)
